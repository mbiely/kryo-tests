...
//    val g5kOpts = SettingKey[String]("g5k-oarsub-options")
  ...
  val g5ksettings = Seq(
    g5kDeploy <<= (streams,g5kDst,g5kUser,g5kSite,g5kRunAll,g5kRunOne,    
		   fullClasspath in Runtime, packageBin in Compile,mainClass in Runtime) map { 
      (out,dst,user,sitename,all,one,cp :Classpath, binJar,main) =>
	...
	scp(log,user,site,joinFiles(jars),dst+"/deps/")
	scp(log,user,site,binJar.toString,dst)
	val cd = join("cd",dst)
